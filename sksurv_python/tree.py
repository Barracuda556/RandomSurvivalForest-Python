...
from sksurv_python._criterion import LogrankCriterion, get_unique_times

# Импортируем наши собственные классы сплиттеров вместо sklearn
from sksurv_python._splitter import (
    Splitter, BestSplitter, BestSparseSplitter, RandomSplitter, RandomSparseSplitter
)
from sksurv_python._tree import BestFirstTreeBuilder, DepthFirstTreeBuilder, Tree
from sksurv_python import _tree
from sksurv_python._utils import _any_isnan_axis0
...
class SurvivalTree(BaseEstimator, SurvivalAnalysisMixin):
    ...
    def _fit(self, X, y, sample_weight=None, check_input=True, missing_values_in_feature_mask=None):
        ...
        if self.low_memory:
            self.n_outputs_ = self.unique_times_.shape[0]  # Исправлено: количество временных точек
            # one value per unique time for CHF
        ...
        if issparse(X):
            if self.splitter == "best":
                ...
                splitter_class = RandomSparseSplitter
        else:
            if self.splitter == "best":
    ...
    def predict(self, X, check_input=True):
        ...
        if self.low_memory:
            ...
            # В режиме low_memory, pred имеет форму (n_samples, n_unique_times)
            # Нужно просуммировать только в моменты событий
            if hasattr(self, 'is_event_time_') and self.is_event_time_ is not None:
                event_time_mask = self.is_event_time_
                # Суммируем только в моменты событий
                risk_scores = pred[:, event_time_mask].sum(axis=1)
            else:
                # Если is_event_time_ не определен, суммируем все времена
                risk_scores = pred.sum(axis=1)
            
            return risk_scores

        # Если не low_memory, используем cumulative hazard function
        chf = self.predict_cumulative_hazard_function(X, check_input, return_array=True)
        
        # Суммируем CHF только в моменты событий
        if hasattr(self, 'is_event_time_') and self.is_event_time_ is not None:
            return chf[:, self.is_event_time_].sum(axis=1)
        else:
    ...
    def predict_cumulative_hazard_function(self, X, check_input=True, return_array=False):
        ...
        pred = self.tree_.predict(X)
        # В режиме low_memory=False возвращается массив с двумя измерениями для CHF и survival
        # Нужно выбрать только CHF
        if pred.shape[-1] == 2:
            # Если последнее измерение равно 2, значит это [CHF, survival]
            arr = pred[..., 0]
        else:
            # Иначе это только CHF
            arr = pred
    ...
    def predict_survival_function(self, X, check_input=True, return_array=False):
        ...
        # В режиме low_memory=False возвращается массив с двумя измерениями для CHF и survival
        if pred.shape[-1] == 2:
            arr = pred[..., 1]  # survival function
        else:
            # Если нет survival function, вычисляем из CHF
            chf = pred
            arr = np.exp(-chf)  # S(t) = exp(-H(t))
